# docker compose -file containers/alarmAnomaly/docker-compose.yml -p "project" up -d --build  
...
CHALLENGE_ROOT = Path("./containers")

BASE_PORT, MAX_PORT = 17010, 17999
...
def resolve_challenge_dir(docker_challenge_id: str) -> Path:
    ...
    log(f"Resolving challenge dir for dockerChallengeID='{docker_challenge_id}' -> {cdir}")
    if not cdir.is_dir():
...
if __name__ == "__main__":
    ...
    # Run the main blocking loop on the primary thread
    process_binlog_event()

    # NOTE: Since process_binlog_event() is an infinite blocking loop, 
    # the thread will be held here. No need for a final 'while True: time.sleep(1)' loop.